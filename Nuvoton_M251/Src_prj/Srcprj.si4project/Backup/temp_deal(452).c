...
void AD_fun_Init_1(ADC_Config *p)
{
	p->ReadADC = EADC_FunctionTest;
	p->Delay_us = delay_Us;	
}
...
uint_16 Temp_deal_fun(ADC_Config *p, uint_8 channel)
{
	...
	temp_dis = EADC_FunctionTest(channel);
...
void Seat_temp_deal(ADC_Config *p)
{
	...
	uint_8  Adj_temp = 0;
	ADtemp = Temp_deal_fun(p, 0);  // 单独输入一个AD通道进行AD采集 ，此处的AD通道为通道0
	//ADtemp = temp_save[0];  //取出处理好的座温温度AD值
    if(ADtemp > 100)
    {
...
void Fan_Temp_Deal(ADC_Config *p)
{
	...
	uint_8 Envirt_index;
	//ADtemp = temp_save[1];
	ADtemp = Temp_deal_fun(p, 1);   // 单独输入一个AD通道进行AD采集 ，此处的AD通道为通道1
